...
**Revision 1 — Waiting-time definition (supersedes §C):**
Every sampled event's `time:timestamp` is truncated to midnight (`T00:00:00.000+01:00`) — event timestamps have **day-level resolution only**, not clock-time. This is stronger than the original "may absorb some service time" caveat; it means raw inter-event gaps are frequently and spuriously zero whenever multiple activities land on the same day, and same-day event order in the file cannot be trusted as true sequence.
**Decision:** `Wait_p,i` is now computed at **activity-day granularity**: collapse each case's same-day events into one activity-day unit, then measure the calendar-day gap between consecutive distinct activity-days. Trace-level DTC `Start date`/`End date` fields (which do carry `HH:MM:SS` precision) are used only as a secondary, coarser cross-check on total episode span — not as the primary waiting signal, since they're per-diagnosis-treatment-combination, not per-activity.
...
**Revision 4 — Discovery alphabet for the reference process model (supersedes §E's implicit assumption):**
624 distinct activities is too high-cardinality for Inductive Miner to produce an interpretable process model — it would yield an unreadable/flower model and near-meaningless deviation costs.
**Decision:** the reference process model (for conformance/`D_p`) is discovered at the **`org:group`** (department) level instead of raw `concept:name` (e.g., `Radiotherapy → Nursing ward → Internal Specialisms clinic`), which is both tractable and clinically interpretable, and conveniently doubles as the "resource" field in the standardized schema (§11). Full `concept:name` granularity is retained separately for the Rework component and for detailed Process Explorer views in the dashboard.
...
`Number of executions` was `"1"` in every sampled event, but the sample was small. If this field takes values >1 elsewhere in the file, the current Rework formula (`row count − 1`) would **undercount** true repetition, since BPIC 2011 may sometimes collapse repeated executions into a single event row with a count rather than emitting separate rows. **The Rework formula (§D) is not finalized until this is checked against the full file** — see `inspect_xes_followup.py`.
